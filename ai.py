...
initial_prompt = (
    ...
    "Ask one question at a time based on these categories to gather comprehensive information about their symptoms. "
    "Always include a disclaimer encouraging users to seek professional medical advice for a proper diagnosis and treatment. "
    "Every time the user says they have another symptom or problem, forget about the previous symptom mentioned."
)
...
def is_new_symptom(user_input):
    # Check if the user mentions a new symptom or problem
    keywords = ["another problem", "another symptom", "different issue", "new symptom", "new problem", "new pain", "new issue", "new concern"]
...
def getResponse(state, user_input):
    if is_new_symptom(user_input):
        state = {"step": "start"}  # Reset the state if a new symptom is mentioned

    if not state or state["step"] == "start":
        state = {"step": "Background"}
        ai_response = questions["Background"]
    else:
        state[state["step"]] = user_input
